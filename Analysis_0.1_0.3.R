...
setwd("C:/ValenteGitHub/ICP/Lucas Data Exploration/Point Count Methodology/wothSimulations/results/Dens0.1_Detect0.3")
...
for (i in 1:1000) { 
  ...
  file_name <- paste0("Occ.1Detect.3Results", i, ".csv")
...
write.csv(combined_results_summary, file = "Full_Results_Summary.1.3.csv", row.names = FALSE)
...
################################################################################
################################################################################
full_results.1.3 = read.csv("Full_Results_Summary.1.3.csv")

# Rename columns directly
colnames(full_results.1.3)[colnames(full_results.1.3) == "ExpDec_DailyBias"] <- "ExponentialDecay_DailyBias"
colnames(full_results.1.3)[colnames(full_results.1.3) == "ExpDec_SeasonBias"] <- "ExponentialDecay_SeasonBias"
colnames(full_results.1.3)[colnames(full_results.1.3) == "ExpDec_InstBias"] <- "ExponentialDecay_InstBias"
colnames(full_results.1.3)[colnames(full_results.1.3) == "Initial_InstantBias"] <- "Initial_InstBias"

# Trim
full_results.1.3_trimmed <- full_results.1.3 %>%
...
full_results.1.3_long <- full_results.1.3_trimmed %>%
  pivot_longer(
    cols = c(
      Initial_InstBias, Initial_DailyBias, Initial_SeasonBias,
      ExponentialDecay_success, ExponentialDecay_DailyBias, ExponentialDecay_SeasonBias, ExponentialDecay_InstBias,
...
summary_results <- full_results.1.3_long %>%
...
# save
write.csv(summary_results, file = "bias_results.1.3.csv", row.names = FALSE) 
