...
setwd("C:/ValenteGitHub/ICP/Lucas Data Exploration/Point Count Methodology/wothSimulations/results/Dens0.2_Detect0.3")
...
for (i in 1:1000) { 
  ...
  file_name <- paste0("Occ.2Detect.3Results", i, ".csv")
...
grouped_data <- combined_df %>%
  ...
  filter(n() == 30) %>%  # Ensure each simulation has exactly 30 rows
  ungroup()
...
fit_model <- function(data, sim_index, model_formula, start_list, model_name) {
  tryCatch({
...
setwd("C:/ValenteGitHub/ICP/Lucas Data Exploration/Point Count Methodology/wothSimulations/results/final")

# Save the combined data frame as a CSV
write.csv(combined_results_summary, file = "Full_Results_Summary.2.3.csv", row.names = FALSE)
...
################################################################################
################################################################################
full_results.2.3 = read.csv("Full_Results_Summary.2.3.csv")

# Rename columns directly
colnames(full_results.2.3)[colnames(full_results.2.3) == "ExpDec_DailyBias"] <- "ExponentialDecay_DailyBias"
colnames(full_results.2.3)[colnames(full_results.2.3) == "ExpDec_SeasonBias"] <- "ExponentialDecay_SeasonBias"
colnames(full_results.2.3)[colnames(full_results.2.3) == "ExpDec_InstBias"] <- "ExponentialDecay_InstBias"
colnames(full_results.2.3)[colnames(full_results.2.3) == "Initial_InstantBias"] <- "Initial_InstBias"

# Trim
full_results.2.3_trimmed <- full_results.2.3 %>%
...
full_results.2.3_long <- full_results.2.3_trimmed %>%
  pivot_longer(
    cols = c(
      Initial_InstBias, Initial_DailyBias, Initial_SeasonBias,
      ExponentialDecay_success, ExponentialDecay_DailyBias, ExponentialDecay_SeasonBias, ExponentialDecay_InstBias,
...
summary_results <- full_results.2.3_long %>%
...
# save
write.csv(summary_results, file = "bias_results.2.3.csv", row.names = FALSE) 